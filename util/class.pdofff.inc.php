...
class Pdofff
{
    ...
    */
    ...
    {
        try {
            $req = "CALL insertClub('".$nom."', '".$ville."', '$nomdirigeant', '".$prenomdirigeant."');";
            Pdofff::$monPdo->query($req);
            //echo "Code retour " . Pdofff::$monPdo->errorCode();
            $mesErreurs = Pdofff::$monPdo->errorInfo();
            if ($mesErreurs[0] == 23000) {
                echo "<br>".$mesErreurs[2].'</br>';
    ...
    */
    public function modifierclub($idc, $nom, $ville, $nomdirigeant, $prenomdirigeant)
    {
        try {
            ...
            $req = "CALL updateClub('".$idc."','".$nom."', '".$ville."', '$nomdirigeant', '".$prenomdirigeant."');";
            Pdofff::$monPdo->exec($req);
            $mesErreurs = Pdofff::$monPdo->errorInfo();
            if ($mesErreurs[0] == 23000) {
                echo "<br>".$mesErreurs[2].'</br>';
            }
            //echo "Correspondant à : " . $mesErreurs;
        } catch (PDOException $e) {
            echo 'Problème SQL ' . $e->getMessage();
            exit;
        }
